...
const completeCodeUrl = getCodeUrl();
const active = ref(DEFAULT_TAB);
const isView = computed(() => active.value === TAB_TYPE.VIEW);

const emitEvent = () => {
  emit('tab-change', active.value);
};

const eventClickView = () => {
  active.value = TAB_TYPE.VIEW;
  emitEvent();
};

const eventClickCode = () => {
  active.value = TAB_TYPE.CODE;
  emitEvent();
};
</script>
...
<template>
  <div class="tab">
    ...
    <a
      v-show="!isView"
      class="complete-code-url"
      target="_blank"
      v-bind:href="completeCodeUrl"
      >COMPLETE CODE</a
    >
  </div>
</template>
